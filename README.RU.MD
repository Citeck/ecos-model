...
Добро пожаловать в репозиторий Citeck `ecos-model`! Репозиторий содержит определения моделей и конфигурации для платформы Citeck - системы для автоматизации бизнес-процессов и управления корпоративными данными.
...
Если ваша IDE поддерживает прямой запуск Spring Boot-приложений, вы можете просто запустить класс 'ru.citeck.ecos.model.EcosModelApp' без дополнительных настроек.
...
Чтобы убедиться, что всё работает, остановите оригинальный контейнер ecos-model и запустите ecos-model:custom вместо него.
...
Если вам нужна помощь или у вас есть вопросы по Citeck `ecos-model`, то создайте issue в этом репозитории или обратитесь в нашу [службу поддержки](mailto:support@citeck.ru).

## Лицензия

Citeck `ecos-model` распространяется под лицензией [GNU Lesser General Public License](LICENSE).
