...
timestamps {
  node {
    ...
    def repoUrl = "git@gitlab.citeck.ru:citeck/ecos-model.git"
    ...
    stage('Checkout Script Tools SCM') {
      dir('jenkins-script-tools') {
        checkout([
          ...
          extensions: [],
          submoduleCfg: [],
          userRemoteConfigs: [[credentialsId: 'awx.integrations', url: 'git@gitlab.citeck.ru:citeck/pipelines.git']]
        ])
      }
    }
    ...
    try {
      ...
      stage('Build project artifacts') {
        withMaven(mavenLocalRepo: '/opt/jenkins/.m2/repository', tempBinDir: '') {
          sh "mvn clean package -Dskip.npm -Pprod -P!webpack"
          // build-info
          ...
          // /build-info
          sh "mvn jib:dockerBuild -Dskip.npm -Pprod -P!webpack -Djib.docker.image.tag=${lower_version}"
        }
        junit '**/target/surefire-reports/*.xml'
      }
      stage('Push docker image') {
        docker.withRegistry('http://127.0.0.1:8082', '7d800357-2193-4474-b768-5c27b97a1030') {
          def microserviceImage = "ecos-model"+":"+"${lower_version}"
